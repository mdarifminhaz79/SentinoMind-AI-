...
class ImageGenerator:
    ...
    def generate_image_binary(self, prompt):
        ...
        pol_url = (
            f"https://enter.pollinations.ai/prompt/{encoded_p}"
    ...
    def generate_and_update(self, unique_id, category, context):
        """ইমেজ জেনারেট, স্টোরেজ আপলোড এবং ডাটাবেস আপডেট করার মূল ফাংশন"""
        logger.info(f"🚀 Processing ID: {unique_id} | Category: {category}")
        
        # ১. সেফ প্রম্পট জেনারেট করুন
        raw_prompt = self.prompt_engine.generate_safe_prompt(category, context)
        if not raw_prompt:
            logger.error(f"❌ Failed to generate prompt for {unique_id}")
            return False

        # ২. প্রম্পট স্যানিটাইজ করুন (মানুষের নাম সরান)
        sanitized_prompt = self.final_sanitize(raw_prompt)

        # ৩. ইমেজ বাইনারি জেনারেট করুন
        image_binary = self.generate_image_binary(sanitized_prompt)
        
        if image_binary:
            # ৪. ইমেজ কোয়ালিটি জাস্টিফাই করুন (ঐচ্ছিক কিন্তু আপনার কোডে আছে)
            verdict = self.justify_visual(context[:100], image_binary)
            if verdict == "FAIL":
                logger.warning(f"⚠️ Image quality check failed for {unique_id}, but proceeding...")

            # ৫. স্টোরেজে আপলোড করুন (public_url পাবেন)
            file_name = f"generated_images/{unique_id}.jpg"
            public_url = self.db.upload_to_storage(file_name, image_binary)
            
            if public_url:
                # ৬. ডাটাবেস আপডেট করুন (public_url এবং sanitized_prompt পাঠাচ্ছেন)
                success = self.db.update_processed_image(unique_id, public_url, sanitized_prompt)
                
                if success:
                    logger.success(f"🎉 Fully Processed & Saved: {unique_id}")
                    return True
                else:
                    logger.error(f"❌ Database update failed for {unique_id}")
            else:
                logger.error(f"❌ Storage upload failed (public_url not found) for {unique_id}")
        else:
            logger.error(f"❌ Image generation failed for {unique_id}")
